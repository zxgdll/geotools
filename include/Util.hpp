...
namespace geotools {
	...
	namespace util {
		...
		private:
			...
			Bounds() :
				Bounds(DBL_MAX_NEG, DBL_MAX_NEG, DBL_MAX_POS, DBL_MAX_POS, DBL_MAX_NEG, DBL_MAX_POS) {
			}
			Bounds(double minx, double miny, double maxx, double maxy) :
				Bounds(minx, miny, maxx, maxy, DBL_MAX_NEG, DBL_MAX_POS) {
			}
		...
		public:
			 ...
			 */
			static bool computeLasBounds(liblas::Header &hdr, std::vector<double> &bounds, int dims) {
			 ...
			 */
			static bool computeLasBounds(liblas::Reader &rdr, std::vector<double> &bounds, int dims) {
				if(dims != 2 && dims != 3)
					return false;
				double x, y, z;
			...
			static void printBounds(std::vector<double> &bounds, int dims) {
				if(dims == 2 || dims == 3) {
					if(dims != 3) {
						_trace("Bounds: " << bounds[0] << "," << bounds[1] << "," << bounds[2] << "," << bounds[3]);
					} else {
						_trace("Bounds: " << bounds[0] << "," << bounds[1] << "," << bounds[2] << "," << bounds[3] << "," << bounds[4] << "," << bounds[5]);
					}
					std::cerr << std::endl;
				}
			}
