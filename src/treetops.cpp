...
void Treetops::smooth(const TreetopsConfig &config) {
	...
	Raster<float> in(config.smoothInputFile);
	Raster<float> out(config.smoothOutputFile, 1, in);
...
void Treetops::treetops(const TreetopsConfig &config) {
	...
	Raster<float> raster(config.topsInputFile);
	Raster<float> smoothed(config.topsSmoothedInputFile);

	// Prepare database.
	g_debug(" -- tops: preparing db");
	std::map<std::string, int> fields;
	fields["id"] = 1;
	SQLite db(config.topsOutputFile, SQLite::POINT, config.srid, fields); // TODO: Get SRID from raster.
	db.makeFast();
	db.dropGeomIndex();
	...
	uint32_t cachedRows = g_max(100, (config.rowCacheSize / raster.rows() / sizeof(float))); // TODO: Make row cache configurable.
	uint32_t blockHeight = config.topsWindowSize * 2 + cachedRows;
	uint32_t numBlocks = raster.rows() / blockHeight;
	uint64_t topId = 0;
	uint64_t topCount = 0;
			...
			for(uint32_t r = 0; r < blockHeight; ++r) {
				for(int c = 0; c < raster.cols(); ++c) {
					...
					if(blk.get(c, r) >= config.topsMinHeight && 
						...
						double umax = raster.get(c, r + curRow);
						...
						std::unique_ptr<Top> t(new Top(
							topId,
							raster.toCentroidX(c + config.topsWindowSize / 2), // center of pixel
							raster.toCentroidY(r + curRow + config.topsWindowSize / 2), 
							max,
							umax,
...
void Treetops::treecrowns(const TreetopsConfig &config) {
	...
	Raster<float> inrast(config.crownsInputFile);
	Raster<uint32_t> outrast(config.crownsOutputRaster, 1, inrast);
	outrast.nodata(0);
	outrast.fill(0);
	...
	SQLite db(config.crownsTreetopsFile);
	uint64_t geomCount;
