 ...
 */
bool Util::computeLasBounds(liblas::Header &hdr, geotools::util::Bounds &bounds, int dims) {
 ...
 */
bool Util::computeLasBounds(liblas::Reader &rdr, geotools::util::Bounds &bounds, int dims) {
	if(dims != 2 && dims != 3)
		return false;
	bounds.collapse(dims == 3);
